<template>
  <div class="simulation1">
    <h1 style="text-align: center">
      {{ $t(isSecond ? 'simulation2.index' : 'simulation1.index') }}: {{ $t(isSecond ? 'simulation2.label' : 'simulation1.label') }}
    </h1>
    <div class="container">
      <div class="left">
      ...
      <div class="right" v-if="isSecond">
        ...
        <h3>{{ $t('simulation2.up') }}: {{ top }}
          <template v-if="top+bottom>0">({{ $t('simulation2.equals') }} {{ (top / Math.max(top + bottom, 1) * 100).toFixed(2) }}%)</template>
        </h3>
        <h3>{{ $t('simulation2.down') }}: {{ bottom }}
          <template v-if="top+bottom>0">({{ $t('simulation2.equals') }} {{ (bottom / Math.max(top + bottom, 1) * 100).toFixed(2) }}%)</template>
        </h3>
        <button class="spreadButton" :disabled="currentlyAnimatedElectrons !== 0" style="margin-top: 20px;" @click="reset()">
          {{ $t('simulation2.reset') }}
        </button>
      </div>
    </div>
...
function shootSingleElectron() {
  ...
  const spinUp = gaussianRandom(0, 5) > 0;
